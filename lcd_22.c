...
void init_lcd(void)
{
	...
	bcm2835_delay( 80 );
	/* Display control */   
	post_cmd( 0x001, 0x0000 ); // 0x0100 for regular
	post_cmd( 0x002, 0x0000 );
	post_cmd( 0x003, 0x12B8 ); // 0x1230 for regular
...
void DisplayChar(u8 casc,u8 postion_x,u8 postion_y)
{
	...
	en_lcd();
	// post_cmd(YS,postion_x*8); 	//x start point
	// post_cmd(XS,postion_y*16); 	//y start point
	// post_cmd(YE,postion_x*8+7);	//x end point
	// post_cmd(XE,postion_y*16+15);	//y end point
	// post_cmd(YP,postion_x*8);	
	// post_cmd(XP,postion_y*16);
  post_cmd(XS,  postion_x*8);   //x start point rotated
  post_cmd(YS,  postion_y*16);  //y start point rotated
  post_cmd(XE,  postion_x*8+7); //x end point rotated
  post_cmd(YE,  postion_y*16+15); //y end point rotated
  post_cmd(XP,  postion_x*8); 
  post_cmd(YP,  postion_y*16);	
...
u8 draw_lcd(void)
{
	...
	post_cmd(YS,touch_xy_buffer[touch_rd_index].x);
	post_cmd(XS,touch_xy_buffer[touch_rd_index].y);
	post_cmd(YE,touch_xy_buffer[touch_rd_index].x+(DOT_WIDTH-1));
	post_cmd(XE,touch_xy_buffer[touch_rd_index].y+(DOT_WIDTH-1));
...
/*void LCD_QQ()
{
        ...
        for(g=0;g<2;g++)
	{
        for(c=0;c<1;c++)
        {
            post_cmd(YS,c*240);
            post_cmd(XS,g*160);
            post_cmd(YE,c*240+239);
            post_cmd(XE,g*160+159);
